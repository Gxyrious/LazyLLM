...
#   bash scripts/synthesize-audio.sh --stop-on-fail # stop at first failure
# ─────────────────────────────────────────────────────────────────────
set -euo pipefail
...
while IFS= read -r row; do
  ...
  if mmx speech synthesize $VOICE_ARG --text "$text" --out "$out" \
    ...
    if [[ "$STOP_ON_FAIL" == true ]]; then
      echo
      echo "✗ stopped after first failure — synthesized $synthesized, skipped $skipped, failed $failed" >&2
      exit 2
    fi
  fi
done < <(jq -c '.[]' "$SEGMENTS")
