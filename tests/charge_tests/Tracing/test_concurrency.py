...
def test_parallel_tracing_propagates_context_across_threads(exporter):
    ...
    assert {s.parent.span_id for s in child_spans} == {parallel_span.context.span_id}
    assert any(
        left.start_time < right.end_time and right.start_time < left.end_time
        for index, left in enumerate(child_spans)
        for right in child_spans[index + 1:]
    )
...
def test_loop_with_parallel_keeps_nested_parent_context(exporter):
    ...
    assert {s.name for s in parallel_spans} == {"Parallel"}

    assert {s.context.trace_id for s in spans} == {loop_span.context.trace_id}
    assert {s.parent.span_id for s in parallel_spans} == {loop_span.context.span_id}
